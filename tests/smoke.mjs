...
assert.match(html, /data-scales-chip/);
assert.match(html, /function scalePickerMarkup\(\)/);
assert.match(html, /Position boxes:/);
assert.doesNotMatch(html, /id="scaleBar"|renderScaleBar/);
assert.doesNotMatch(html, /scaleOverlayStatus/);
...
assert.match(html, /leadDisplay==='tab'\?String\(f\):scaleNoteName\(overlayScale\.root,overlayScale\.scale,scaleIndex\)/);
...
assert.match(html, /function leadPhraseGroups\(events\)/);
assert.match(html, /function leadPhraseLabel\(phrase\)/);
assert.match(html, /function leadPhrasePlayMode\(phrase\)/);
assert.match(html, /function leadPhrasePlayModeSelect\(phrase,onChange\)/);
assert.match(html, /Multi-note linked slides, hammer-ons, and pull-offs default to <b>Strum<\/b>/);
assert.match(html, /function validateLeadConnection\(sourceNotes,destination,action\)/);
assert.match(html, /className='lead-event'\+\(isPhrase\?' lead-phrase':''\)/);
assert.match(html, /function renderLeadPhraseEditor\(\)/);
...
assert.match(html, /Repeat this entire solo phrase/);
assert.match(html, /leadPhraseEdit=\{start:phrase\.start,end:phrase\.end,active:null\}/);
assert.match(html, /event\.tabIndex=0;event\.setAttribute\('role','button'\)/);
assert.match(html, /const priorDraft=copyLeadEvents\(leadDraft\)/);
assert.doesNotMatch(html, /lead-link-source|lead-link-target/);
assert.doesNotMatch(html, /cue\.textContent='starts '|link\.textContent='from '/);
...
assert.match(html, /one dashed phrase/);
assert.match(html, /add another destination to make a longer chain/);
...
assert.match(html, /async function scanLinkedFolder\(opts\)/);
assert.match(html, /Clear song/);
assert.doesNotMatch(html, /id="btnSaveFresh"/);
assert.doesNotMatch(html, /id="btnBackup"|id="btnImport"/);
...
assert.match(html, /function playLeadConnectionPreview\(sourceByString,technique,links,destinationNotes(?:,playMode='strum')?\)/);
assert.match(html, /const spacing=mode==='strum'\?0:mode==='arp'\?0\.12:0/);
